...
<template>
    <div id="image-waterfall"
        ...
        <div class="header"
            ...
            <el-select v-model="selectedSizeId"
                       size="mini"
                       placeholder="尺寸筛选"
                       :disabled="loading"
                       @change="handleLoadMoreClick(true)">
                <el-option v-for="item in selectionSize"
                           :key="item.id"
                           :label="item.name"
                           :value="item.id">
                </el-option>
            </el-select>
            <el-select v-model="selectedTagsId"
                       size="mini"
                       placeholder="标签筛选"
                       :disabled="loading"
                       @change="handleLoadMoreClick(true)">
                <el-option v-for="item in selectionTags"
                           :key="item.id"
                           :label="item.name"
                           :value="item.id">
                </el-option>
            </el-select>
            <el-select v-model="selectedBangumiId"
                       size="mini"
                       placeholder="番剧筛选"
                       filterable
                       :disabled="loading"
                ...
                <el-option v-for="item in selectionBangumis"
                           :key="item.id"
                           :label="item.name"
                           :value="item.id">
                </el-option>
            ...
            <el-select v-model="selectedRoleId"
                ...
                <el-option v-for="item in selectionRoles"
                           :key="item.id"
                           :label="item.name"
                           :value="item.id">
                </el-option>
            </el-select>
            <el-select v-model="selectedCreatorId"
                       size="mini"
                       placeholder="原创分类"
                       :disabled="loading"
                       @change="handleLoadMoreClick(true)">
                <el-option v-for="item in creators"
                           :key="item.id"
                           :label="item.name"
                           :value="item.id">
                </el-option>
        ...
        <no-ssr>
            <waterfall class="image-container"
                       :line-gap="212"
                       :auto-resize="false">
                <waterfall-slot v-for="(item, index) in list"
                    ...
                    <div class="image">
                        <div class="item-wrap"
                            ...
                            <i v-if="item.creator"
                               class="creator iconfont icon-huangguan"></i>
                            <div v-if="computeOptions(item).length"
                                 class="menu"
                                 @click.stop.prevent>
                        ...
                        <div class="desc">
                            <a class="name oneline"
                               href="javascript:;"
                               target="_blank"
                               v-text="item.name"
                               v-if="item.image_count"></a>
                            <div class="tags"
                                 v-else>
                                <button class="el-tag oneline"
                                        v-text="item.size.name"></button>
                                <button class="el-tag oneline"
                                        v-for="tag in item.tags"
                                        :key="tag.id"
                                        v-text="tag.name"></button>
                            </div>
                            <div class="meta">
                                <button class="like"
                        ...
                        <div class="detail user clearfix"
                            ...
                            <div class="info">
                                <a class="oneline"
                                   ...
                                   :href="$alias.user(item.user.zone)"
                                   target="_blank"
                                   v-text="item.user.nickname"></a>
                                <a class="oneline"
                                   v-if="item.bangumi_id"
                                   :href="$alias.bangumi(item.bangumi.id)"
                                   target="_blank"
                                   v-text="item.bangumi.name"></a>
                            </div>
                        </div>
                        <template v-else-if="page === 'user-show'">
                            <div class="detail bangumi clearfix"
                                 v-if="item.bangumi_id">
                                <a class="avatar"
                                   :href="$alias.bangumi(item.bangumi.id)"
                                   target="_blank">
        ...
        <el-button :loading="loading"
                   ...
                   type="info"
                   plain>{{ loading ? '加载中' : '加载更多' }}</el-button>
        <!--编辑图片弹窗-->
        <v-modal v-model="openEditModal"
                 header-text="编辑图片"
                    ...
                    <el-col :span="10">
                        <el-form-item label="类型">
                            <el-select v-model="form.tags"
                                       placeholder="请选择类型">
                                <el-option v-for="item in options.tags"
                                           :key="item.id"
                                           :label="item.name"
                                           :value="item.id">
                                </el-option>
                    ...
                    <el-col :span="10">
                        <el-form-item label="尺寸">
                            <el-select v-model="form.size"
                                       placeholder="请选择尺寸">
                                <el-option v-for="item in options.size"
                                           :key="item.id"
                                           :label="item.name"
                                           :value="item.id">
                                </el-option>
        ...
        <!--编辑专辑弹窗-->
        <v-modal v-model="openEditAlbumModal"
                 header-text="编辑专辑"
            ...
            <el-form ref="form"
                ...
                <el-row v-if="!albumForm.is_cartoon">
                    <el-col :span="10">
                        <el-form-item label="番剧">
                            <el-select v-model="albumForm.bangumiId"
                                       filterable
                                       placeholder="请选择番剧">
                                <el-option v-for="item in bangumis"
                                           :key="item.id"
...
export default {
    ...
    computed: {
        ...
        curUserId() {
            return this.$store.state.login ? this.$store.state.user.id : -1;
        },
        zone() {
    ...
    methods: {
        ...
        computeOptions(image) {
            ...
            if (this.isMine(image.user_id)) {
                result = result.concat(['删除', '编辑']);
            } else {
                result = result.concat(['举报']);
            }
            return result;
        },
        computeBoxHeight(image) {
            return (
                this.computeImageHeight(image) +
                (this.page === 'user-show'
        ...
        async handleAlbumEditDone() {
            ...
            try {
                const data = await api.editAlbum(
                    Object.assign(
                        {
                            id,
        ...
        beforeUpload(file) {
            ...
            this.uploadHeaders.key = `user/${
                this.$store.state.user.id
            }/image/${new Date().getTime()}-${Math.random()
                .toString(36)
