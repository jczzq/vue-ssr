...
<template>
    <div class="v-select-wrap">
        ...
        <div class="v-select-text-area-wrap"
            ...
            <ul class="v-select-selected-item-wrap"
                ...
                <li v-for="item in selectedList"
                    ...
                    <slot name="selected">
                        <span class="v-selected-selected-cancel"
                              v-if="multi"
                              @click="handleSelectedCancel(item)">&times;</span>
                        {{ item[`${showLabel}`] }}
                    </slot>
                </li>
            </ul>
            <div class="v-select-placeholder"
                 v-else-if="!show">
        ...
        <transition name="zoom-in-top">
            <ul class="v-select-options-wrap"
                ref="ul"
                v-show="show">
                <slot name="options">
...
<script>
const privateKeys = ['__selected__', '__disabled__', '__uuid__', '__isObj__'];
const defaultLabels = {
    show: '__label__',
    ...
    computed: {
        ...
        haveSelected() {
            ...
            return temp // eslint-disable-line
                ? this.multi ? Array.isArray(temp) && temp.length : !!temp
                : false;
        }
    },
    data() {
            ...
            this.list.forEach((item, index) => {
                if (Array.isArray(this.value)) {
                    this.value.forEach(value => {
                        if (
                            item[this.showLabel] ===
                            (typeof value === 'string' ? value : value[this.showLabel])
                        ) {
                            this.list[index][this.selectedLabel] = true;
                        }
                    });
                } else if (
                    item[this.showLabel] === (isStr ? this.value : this.value[this.showLabel])
                ) {
                    this.list[index][this.selectedLabel] = true;
                }
            });
