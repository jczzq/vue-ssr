    ...
    <li class="post-show-item">
        <div class="header clearfix">
            ...
            <el-tooltip effect="dark"
                ...
                <span class="time">
                    回复于:
                    <v-time v-model="item.updated_at"></v-time>
                </span>
            </el-tooltip>
            <span class="time"
                  v-else>
            ...
            <a class="title oneline href-fade-blue"
               target="_blank"
        ...
        <p class="content"
           ...
           v-else>
            <a target="_blank"
               :href="$alias.user(item.user.zone)">{{ item.user.nickname }}</a>
            : {{ item.desc }}
        </p>
        <div class="images clearfix"
            ...
            <div class="image-box"
                ...
                <v-img :src="image.url"
                       height="90"
                       mode="2"
                       :aspect="$computeImageAspect(image)"></v-img>
            </div>
        </div>
        <span class="counter"
              v-text="item.comment_count"></span>
        <div class="footer">
            <v-share type="button"
                     :url="`/post/${item.id}`"
                     :title="item.title"
                     :desc="item.desc"></v-share>
            <span :class="{ 'done': item.marked }">
                <i class="iconfont icon-buoumaotubiao44"></i> 收藏数&nbsp;({{ item.mark_count }})
            </span>
            <span :class="{ 'done': item.liked }">
                <i class="iconfont icon-guanzhu"></i> 喜欢数&nbsp;({{ item.like_count }})
            </span>
            <span>
                <i class="iconfont icon-ai-eye"></i> 阅读数&nbsp;({{ item.view_count }})
            </span>
        </div>
    </li>
</template>
