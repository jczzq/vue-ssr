...
const mutations = {
    ...
    SET_SUB_COMMENTS(state, { comments, parentId }) {
        ...
        // 对服务端进行一个校准
        const hasNew = !!resIds.filter(_ => parentCommentList.map(_ => _.id).indexOf(_) === -1)
            .length;
        state.list[parentIndex].comments.list = parentCommentList.filter(
            _ => resIds.indexOf(_.id) === -1
        );
        state.list[parentIndex].comments.list = state.list[parentIndex].comments.list.concat(
            comments.list
        );
                ...
                parent.comments.list.forEach((item, subIndex) => {
                    if (item.id === id) {
                        ...
                        state.list[index].comments[subIndex].like_count =
                            state.list[index].comments[subIndex].like_count + (result ? 1 : -1);
                    }
                });
            }
        });
