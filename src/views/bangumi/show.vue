...
<template>
    <div id="bangumi-show">
        <section id="banner">
            <div class="img bg"
                 :style="{ backgroundImage: info ? `url(${$resize(info.banner, { width: 1920, mode: 0 })})` : '' }"></div>
            <div class="info">
                <h1 class="title"
                    v-text="info.name"></h1>
                <p class="summary"
                   v-text="info.summary"></p>
                <div class="console">
                    <button class="follow"
                            @click="follow"
                            :class="[ info.followed ? 'is-followed' : 'not-follow' ]">
                        <i class="iconfont icon-guanzhu"></i> {{ info.followed ? '已关注' : '关注' }}
                    </button>
                </div>
            </div>
                ...
                <div id="tags"
                    ...
                    <ul class="tags-wrap">
                        <li class="tag"
                            v-for="tag in tags"
                            :key="tag.id">
                            <a :href="$alias.bangumiTag(tag.id)"
                               class="el-tag"
                               v-text="tag.name"
                               target="_blank"></a>
                        </li>
                ...
                <div id="followers">
                    ...
                    <template v-if="info.count_like">
                        <ul>
                            ...
                            <button v-if="info.count_like > 7"
                                    @click="openFollowersModal = true"
                                    class="followers-more-btn el-icon-more"></button>
                        </ul>
                        <v-modal v-model="openFollowersModal"
                            ...
                            <li v-for="user in followers"
                                ...
                                <a :href="$alias.user(user.zone)"
                                   target="_blank">
                                    ...
                                    <v-time class="score"
                                            v-model="user.created_at"></v-time>
                                </a>
                            </li>
                        </v-modal>
                    </template>
            ...
            <div class="col-main">
                <el-tabs @tab-click="handleTabClick">
                    <el-tab-pane label="帖子">
                        <ul id="posts">
                            <post-show-item v-for="item in posts.data"
                                            :key="item.id"
                                            :item="item"></post-show-item>
                        </ul>
                        <el-button :loading="postState.loading"
                                   ...
                                   @click="getPosts"
                                   type="info"
                                   plain>{{ postState.loading ? '加载中' : '加载更多' }}</el-button>
                        <no-content v-if="posts.noMore && !posts.total">
                            <el-button @click="openCreatePostModal"
                                       type="primary"
                                       round>发表《{{ info.name }}》的第一个帖子</el-button>
                        </no-content>
                    </el-tab-pane>
                                            ...
                                            <a :href="$alias.video(video.id)"
                                               target="_blank">
                                                <figure>
                                                    <v-img class="bg"
                                                           :alt="video.name"
                                                           :src="$resize(video.poster, { width: 192, height: 120 })"></v-img>
                                                    <figcaption class="abs">
                                                        <p class="part oneline">第{{ video.part - season.base }}话</p>
                                                        <span class="name"
                                                              v-text="video.name"></span>
                                                    </figcaption>
                                                </figure>
                                            </a>
                                        </li>
                    ...
                    <el-tab-pane label="漫画"
                                 v-if="info.has_cartoon">
                        <div id="cartoons">
                            <ul class="clearfix"
                                v-if="cartoonInfo.list.length">
                                <li v-for="item in cartoonInfo.list"
                                    :key="item.id">
                                    <a class="poster-wrap"
                                    ...
                                    <div class="desc">
                                        <button class="like"
                                            ...
                                            <i class="iconfont icon-guanzhu"></i> {{ item.like_count || '' }}
                                        </button>
                                        <a class="oneline"
                                           :href="$alias.imageAlbum(item.id)"
                                           target="_blank"
                                    ...
                                    <a class="user"
                                       ...
                                       target="_blank">
                                        ...
                                        <div class="oneline"
                                             v-text="item.user.nickname"></div>
                                    </a>
                                </li>
                            </ul>
                            <no-content v-else-if="cartoonInfo.noMore">
                                <el-button @click="openFeedbackForCartoon"
                                           type="primary"
                                           round>求漫画</el-button>
                            </no-content>
                            <el-button :loading="cartoonInfo.loading"
                                       v-if="!cartoonInfo.noMore"
                                       class="load-more-btn"
                                       @click="getCartoons(false)"
                                       type="info"
                                       plain>{{ cartoonInfo.loading ? '加载中' : '加载更多' }}</el-button>
                        </div>
                    </el-tab-pane>
                    <el-tab-pane label="偶像">
                        <div id="roles">
                                ...
                                <li class="role"
                                    ...
                                    <div class="clearfix">
                                        ...
                                        <div class="summary">
                                            ...
                                            <el-button @click="handleStarRole(item)"
                                                       type="warning"
                                                       class="star"
                                                       size="mini"
                                    ...
                                    <div class="footer"
                                        ...
                                        <span>
                                            粉丝: {{ $utils.shortenNumber(item.fans_count) }}
                                        </span>
                                        ·
                                        <span>
                                            金币: {{ $utils.shortenNumber(item.star_count) }}
                                        </span>
                                        ·
                                        ...
                                        <span class="load-list"
                                              @click="showRoleDetail(item, 'new')">最新应援</span>
                                        ·
                                        <span class="load-list"
                                              @click="showRoleDetail(item, 'hot')">最多应援</span>
                                    </div>
                                </li>
                            </ul>
                            <el-button :loading="rolesState.loading"
                                       v-if="!roles.noMore"
                                       class="load-more-btn"
                                       @click="getRoles"
                                       type="info"
                                       plain>{{ rolesState.loading ? '加载中' : '加载更多' }}</el-button>
                            <template v-if="roles.noMore">
                                <div v-if="roles.data.length"
                                     class="load-more-roles">
                                    <el-button @click="openFeedbackForRole"
                                               type="primary"
                                               round>没有你喜欢的角色？</el-button>
                                </div>
                                <no-content v-else>
                                    <el-button @click="openFeedbackForRole"
                                               type="primary"
                            ...
                            <v-modal class="role-fans-modal"
                                ...
                                <li v-for="item in currentRoleFans.data"
                                    :key="item.id">
                                    <a class="lover-user"
                                       target="_blank"
                                       :href="$alias.user(item.zone)">
                                        ...
                                        <v-time class="score"
                                                ...
                                                v-model="item.score"></v-time>
                                        <span class="score"
                                              v-else>{{ item.score }}个金币</span>
                                    </a>
                                </li>
                            </v-modal>
                        </div>
                    ...
                    <el-tab-pane label="相册">
                        <image-waterfall :loading="imagesState.loading"
                                         :role="roles.data"
                                         @fetch="getImages(false)"></image-waterfall>
                    </el-tab-pane>
                </el-tabs>
            </div>
        </div>
